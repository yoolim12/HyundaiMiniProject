 ...
 * page 셋팅하는 스크립트
 * 
 * [totalItemCount : 데이타 총 개수. 필수값] [pagingArea : 페이징 내용이 삽입될 부분. jquery
 * selector 형태. 필수값] [curPageNum : 현재 페이지 번호. default = 1. 필수값 아님] [pagingSize :
 * 페이징 처리될 데이타 개수. default = 10, 필수값 아님] [pageDisplaySize : 화면에 나타날 페이지 개수.
 * default = 10, 필수값 아님]
 * 
 * ========================[example code]===========================
 * 
 ...
 * =================================================================
 * 
 * 실제로 데이타 처리와 연동되는 부분은 구현해서 사용? 해야하려나?..... 오버라이드 해서 사용해야하나...?
 * 
 * 
 */
 ...
 * 게시판 형태 리스트 뿌리는 함수
 * 
 * GET 형태, Responsebody SearchPageData로 리턴하는 컨트롤러 필요, JSP 페이지에 action url이 들어간
 * form 태그 필요 script로 리스트 구성하는 함수를 오버라이드 해서 사용
 * 
 * 
 * ========================[example code]===========================
 * 
 ...
 * @ResponseBody public SearchPageData<OrderData>
 *               getMyOrderList(@RequestParam(value = "pageNum", defaultValue =
 *               "1") final int pageNum,
 * @RequestParam(value = "pageSize", defaultValue = "10") final int pageSize,
 * @RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
 * @RequestParam(value = "sort", required = false) final String sortCode, final
 *                     HttpServletRequest request, final HttpServletResponse
 *                     response) {
 * 
 * HashMap whereValue = new HashMap();
 * 
 * final PageableData pageableData = createPageableData(pageNum - 1, pageSize,
 * sortCode, showMode);
 * 
 * SearchPageData<OrderData> datas = myOrdersFacade.getMyOrders(pageableData,
 * whereValue);
 * 
 * return datas; }
 * 
 ...
 * [script sample]
 * 
 * var orderList = new BoardListController('orderSearchForm', '#listBody',
 * '.paging', 8, 'searchBtn'); 폼아이디, 리스트공간 셀렉터, 페이징공간 셀렉터, 페이지당 리스트개수, 검색버튼 아이디
 * 
 * orderList.setRowHtml = function(results){ var rows = ""; $.each(results,
 * function(){
 * 
 * var listTag = '
 * <tr>'; listTag += '
 * <td>'+this.code+'</td>'; listTag += '
 * <td>'+this.currency+'</td>'; listTag += '
 * <td>'+this.pk+'</td>'; listTag += '
 * <td>'+this.calculated+'</td>'; listTag += '
 * <td></td>'; listTag += '
 * <td></td>'; listTag += '</tr>';
 * 
 * rows +=listTag; });
...
var LayerPopupController = function(layerId){
	 ...
	 * params
	 * 
	 * layerID x좌표, y좌표????????? 클릭 기준점???? (default : center) 입력 내용 유지여부 true,
	 * false
	 * 
	 * 딱히 셋팅하는 부분 없이.
	 * 
...
// PARAMS.layer.css("left",lp).css("top", tp).css("position",
// "absolute").css("z-index", 101);
...
/**
 * 
 * 센터에 레이어 팝업 뜨는 함수 display:none 처리되어있는 팝업 DIV 영역을 CLONE으로 복사해서 사용한다. 따라서 닫을때
 * remove 하고 다시 띄울 때 새로운 창을 띄우므로 닫기전 데이타를 가지고 있지 않는 장점이 있지만 동일한 레이어가 한테이지에 두개가
 * 생기기때문에 요소 접근에 신경을 써야한다.
 * 
 * ========================[example code]=======================
 * layerPopup('레이어영역ID');
 * =============================================================
...
// var lp=($(window).width()-PARAMS.layer.width())/2;
        // var
		// tp=($(window).height()-PARAMS.layer.height())/2+$(window).scrollTop();
 ...
 * 
 * var lc = new layerAlert('입력하신 정보가 변경완료 되었습니다.'); lc.confirmAction =
 * function(){ //alert화면 실행후 후속 처리 부분 추가 location.href="<c:url
 * value="/mypage/order/myorders"/>"; };
 ...
 * ========================[example code]===========================
 * 
 * var lc = new layerConfirm('메세지를 입력하세요', '확인버튼', '취소버튼'); lc.confirmAction =
 * function(){ console.log('재정의한 함수입니다.'); };
 * 
 * =============================================================
 * 
 */
 ...
 * 
 * checkIdList를 배열형태로 셋팅해주면 해당 노드를 찾아서 검사. 아이디 뒤에 :num 이 붙은 경우에는 숫자인지 검사.
 * 
 ...
 * ========================[example code]===========================
 * 
 * var vc = new ValidationCheck(); vc.checkIdList = ['id1:num','id2','id3',
 * 'id4', 'id5']; console.log(vc.isValid());
 * 
 * =============================================================
 * 
 */
 ...
 * ========================[example code]===========================
 * 
 * inputDataValidationCheck('id'); inputDataValidationCheck('password');
 * 
 * =============================================================
 * 
 */
...
var inputDataValidationCheck = function(fid){
    ...
    $('#'+fid).keyup(function(event){
        ...
        /*
		 * if (pw_passed) { pw_msg = "<span class='color:blue;'>* 사용할 수 있는
		 * 비밀번호입니다.</span>"; $("#pw_cert_view").html(pw_msg); }
		 */
...
var capsLock = function(e){
     var keyCode = 0;
      ...
      keyCode=e.which;
      shiftKey=e.shiftKey;
      // alert(shiftKey + "=======" + keyCode + "===============" + ((keyCode
		// >= 65 && keyCode <= 90)&& !shiftKey)||((keyCode >= 97 && keyCode <=
		// 122)&& shiftKey));
 ...
 * Code]*************************************
 * 
 * searchAddrLayer('address1', 'address2', 'zipCode'); 주소1필드아이디, 주소2필드아이디,
 * 우편번호필드아이디
 * 
 * *************************************************************************************
 * 
 ...
 * Code]*************************************
 * 
 * searchAddressLayer('address1', 'address2', 'zipCode'); 주소1필드아이디, 주소2필드아이디,
 * 우편번호필드아이디
 * 
 * *************************************************************************************
 * 
...
var searchAddressLayer = function(addr1, addr2, zipcode){
    ...
    this.openNewLayer = function(){
        ...
        var scrollTopCurrent = $(window).scrollTop();
        $thisLayer = $(layerTag).appendTo($('body')).append(addressLayerTag);
        // $thisLayer.find('.popwrap').css('marginTop',
		// getMarginTop2(830,scrollTopCurrent));
        ...
        $thisLayer.find("#applyBtn").click(function(){
            ...
            if($("#athome_svc").length > 0 && zipcode == "postalcode"){
                ...
                if("060,061,062,063,065,066,067,068,055,056,057,058,039,040,041,042,043,044,069,070,072,073,074,052,053,054".indexOf(checkCode) > -1){
                    ...
                    layerHtml = '앳홈 서비스 불가능 지역 입니다.<br />(강남구/강동구/동작구/마포구<br />/서초구/영등포구/용산구에 한정)';// 17.12
																									// -
																									// 용산/마포구
																									// 추가
        ...
        if(roadSearchProcessCheck){
            ...
            $.ajax({
                ...
                success:function(xmlStr){           // xmlStr : 주소 검색 결과 XML 데이터
					 ...
					 * 환경에서 JSONP의 returnXml 결과 데이터 처리 var xmlData=
					 * newActiveXObject("Microsoft.XMLDOM");
					 * xmlData.loadXML(xmlStr.returnXml); }else{ // IE 이외 환경에서
					 * 처리 var xmlData= xmlStr.returnXml; }
					 */
                    var xmlData= $(xmlStr).find("results");
                    // $(".zipcode_search_result_list").html(""); // 결과 출력 영역
					// 초기화
                    ...
                    /*
					 * var response = data.results; var pagination =
					 * data.pagination;
					 * 
					 * $thisLayer.find('#resultTotalNumber').text('검색 :
					 * '+pagination.totalNumberOfResults+'건'); var listTags =
					 * "";
					 * 
					 * 
					 * $.each(response, function(){ //alert(this.addrDisp); var
					 * row = '<li class="addrListSelect">' +'<a
					 * href="javascript:void(0);" class="clearfix">' +'<span
					 * class="float_left addrIn">도로명 : '+this.addrDisp+'</span>' +'<span
					 * class="float_right">'+this.zipCd+'</span><br/>' +'<span
					 * class="float_left">지번 : '+this.addrDispLot+'</span>' +'</a>' +'</li>';
					 * 
					 * listTags += row;
					 * 
					 * });
    ...
    /*
	 * 모의 해킹 테스트 시 검색API를 호출하시면 IP가 차단 될 수 있습니다. 주소검색API를 제외하시고 테스트 하시기 바랍니다.
	 */
    /*
	 * 도로명주소 API 호출시 검색어에 특수문자 또는 OR, INSERT, UNION 등 SQL예약어가 포함될 경우 보안장비가 SQL
	 * INJECTION공격으로 간주하여 해당 IP를 차단시킬 수 있습니다. 사용자분들은 API호출시 검색어 필터링를 적용하여 주시기
	 * 바랍니다.
	 */
...
/**
 * email domain selectbox controller script 이메일 도메인 셀렉트박스 컨트롤러 스크립트
 * 
 * *********************************[example
 * Code]*************************************
 * 
...
/**
 * ex) wordLengthReplace('가나다라마', 3, '*') return 가나다*
 */
...
/**
 * passwordControll 패스워드관련 동작들 (compare:비교, save:저장)
 * 
 * *********************************[example
 * Code]*************************************
 * 
 * var pwc = new passwordControll("compare", inputPw, prk); division :
 * "compare"(입력PW와 저장되어있는PW 비교), "save" (바뀐 패스워드를 저장) pwc.trueAction =
 * function() { //동작 성공시 실행할 로직들 추가 alert("패스워드가 확인되었습니다. 또는 패스워드를 저장하였습니다."); };
 * pwc.falseAction = function() { //동작 실패시 실행할 로직들 추가 alert("패스워드를 확인하여 주십시오. 또는
 * 패스워드 저장에 실패하였습니다."); };
 * 
 * 
 * *************************************************************************************
...
/**
 * passwordEncoding 패스워드 암호화 (Hybris체계 암호화)
 * 
 * *********************************[example
 * Code]*************************************
 * 
 * encodePW = passwordEncoding(inputPW, userID); 암호화된PW 암호화할PW, 유저ID
 * 
 * *************************************************************************************
 * 
 * 
...
/**
 * 2017-03-08, 이현승, 전화번호 문자포함여부 검증. 검증성공: SUCCESS 검증실패: 해당태그ID
 */
...
/*
 * str : 문자 byteLength : 자를 문자 수 sizePerLetter : 한글일 경우 byte수
 */
