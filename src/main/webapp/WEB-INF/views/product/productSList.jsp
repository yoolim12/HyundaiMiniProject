		...
		<h3 class="cnts_title ou1804">
	        <span><!-- 정상 브랜드 카테고리 목록 -->
        		...
        		<img src="http://cdn.thehandsome.com/_ui/desktop/common/images/products/ou_location_arr.png" alt="location arr">
	           	<a href="/product/productMList?category=${category_m}&pageNum=${1}&amount=${pageMaker.cri.amount}">${category_m}</a>
	           	<img src="http://cdn.thehandsome.com/_ui/desktop/common/images/products/ou_location_arr.png" alt="location arr">
	           	<a href="/product/productSList?category=${category_s}&pageNum=${1}&amount=${pageMaker.cri.amount}">${category_s}</a>
	        </span>
         </h3>
			...
			<ul class="clearfix" id="listBody">
           	...
           	<div class="paging" style="display: block;">
           		<c:if test="${pageMaker.cri.pageNum != 1}">
           			<a class="prev2" href="/product/productSList?category=${category_s}&pageNum=${1}&amount=${pageMaker.cri.amount}">처음 페이지로 이동</a>
           			<a href="/product/productSList?category=${category_s }&pageNum=${pageMaker.cri.pageNum-1}&amount=${pageMaker.cri.amount}" class="prev">이전 페이지로 이동</a>
           		</c:if>
	           	<span class="num">
	            	<c:forEach var="num" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
	            		<a href="/product/productSList?category=${category_s }&pageNum=${num}&amount=${pageMaker.cri.amount}" id="page_button" class="pageBtn ${pageMaker.cri.pageNum == num ? 'on':'' }">${ num}</a>
	  					<!-- 현재 페이지 class: pageBtn on -->
	            	</c:forEach>
	           	</span>
	           	<c:if test="${pageMaker.cri.pageNum != pageMaker.realEnd}">
	           		<a href="/product/productSList?category=${category_s }&pageNum=${pageMaker.cri.pageNum + 1}&amount=${pageMaker.cri.amount}" class="next">다음 페이지로 이동</a>          	
	           		<a href="/product/productSList?category=${category_s }&pageNum=${pageMaker.realEnd}&amount=${pageMaker.cri.amount}" class="next2">마지막 페이지로 이동</a>
	           	</c:if>
           	</div>
